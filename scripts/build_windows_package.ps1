...
Write-Host "Downloading LÖVE $LoveVersion from $loveUrl"
...
if (-not $loveExtracted) {
    throw "Could not find extracted LÖVE runtime folder."
}
...
Copy-Item -LiteralPath (Join-Path $loveExtracted.FullName "*") -Destination $packageRoot -Recurse -Force
Copy-FileBytes (Join-Path $loveExtracted.FullName "love.exe") (Join-Path $packageRoot "noizemaker.exe")
Append-FileBytes (Join-Path $packageRoot "noizemaker.exe") $gameLove

Remove-Item -LiteralPath (Join-Path $packageRoot "love.exe") -Force
